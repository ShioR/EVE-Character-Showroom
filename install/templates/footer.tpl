
<!-- FOOTER -->
<br /><br />
<br /><br />
<br /><br />
<br /><br />
<br /><br />
<div style="margin: auto auto 20px; width: 70%;" alt="Copyright">
    <div style="text-align:center; color: grey;">All images and logos are Copyright &copy; <a style="text-decoration:none; color:grey;" title="CCP hf" href="http://ccpgames.com/">CCP hf</a></div>
    <div style="text-align: center; font-size: x-small;"><a style="text-decoration: none;color:grey;" target="_blank" href="https://github.com/ShioR/EVE-Character-Showroom/issues/new" title="Bugs">Submit Bug</a></div>
</div>
</body>
</html>